...
const _SPECIAL_URL_PREFIXES = [
    'chrome://', 'chrome-extension://', 'moz-extension://',
    'about:', 'edge://', 'opera://', 'file://'
];
const _SPECIAL_HOSTNAMES = [
    'addons.mozilla.org', 'accounts-static.cdn.mozilla.net',
    'accounts.firefox.com', 'addons.cdn.mozilla.net',
...
/**
 * Get current active tab info (domain + tabId).
 * @returns {Promise<{domain: string, tabId: number}|null>}
 */
async function getCurrentTab() {
    try {
        if (!browser.tabs || !browser.tabs.query) return null;
        const tabs = await browser.tabs.query({ active: true, currentWindow: true });
        if (!tabs || tabs.length === 0) return null;
        const tab = tabs[0];
        if (!tab.url) return null;
        if (_SPECIAL_URL_PREFIXES.some(p => tab.url.startsWith(p))) return null;
        try {
            const urlObj = new URL(tab.url);
            if (_SPECIAL_HOSTNAMES.includes(urlObj.hostname)) return null;
...
/**
 * Load temporary tab whitelist domains for the active tab from the background.
 * @param {number} tabId
 * @returns {Promise<string[]>}
 */
async function loadTemporaryTabWhitelist(tabId) {
    try {
        const response = await browser.runtime.sendMessage({
            function: "getTemporaryTabWhitelistForTab",
            params: [tabId]
        });
...
/**
 * Tell the background to add a domain to the in-memory temporary tab whitelist.
 */
async function addToTemporaryTabWhitelistBackground(domain, tabId) {
    try {
        await browser.runtime.sendMessage({
            function: "addToTemporaryTabWhitelist",
            params: [domain, tabId]
        });
        return true;
    } catch (e) {
...
/**
 * Tell the background to remove a domain from the in-memory temporary tab whitelist.
 */
async function removeFromTemporaryTabWhitelistBackground(domain, tabId) {
    try {
        await browser.runtime.sendMessage({
            function: "removeFromTemporaryTabWhitelist",
            params: [domain, tabId]
        });
        return true;
    } catch (e) {
...
async function handleDynamicWhitelistToggle() {
    ...
    if (button) {
        ...
        if (success) {
            ...
            setTimeout(() => button.classList.remove('whitelist-feedback-success'), 800);
        } else {
            const unicodeDomain = domainToUnicode(domain);
            const displayDomain = asciiDomain !== unicodeDomain ? `${unicodeDomain} (${asciiDomain})` : domain;
            const errorMsg = translate('whitelist_update_failed') || `Failed to update whitelist for ${displayDomain}`;
            alert(errorMsg);
        }
    }
}
...
async function refreshAllData() {
    try {
        ...
        if (tabInfo) {
            ...
            temporaryTabWhitelistDomains = await loadTemporaryTabWhitelist(tabInfo.tabId);
        }
