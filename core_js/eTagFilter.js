...
function eTagFilter(requestDetails) {
    ...
    // Explicit temporary tab whitelist check.
    if (
        typeof getTemporaryTabWhitelistForTab === 'function' &&
        typeof requestDetails.tabId === 'number' &&
        requestDetails.tabId >= 0
    ) {
        try {
            const hostname = new URL(requestDetails.url).hostname.toLowerCase();
            const tempDomains = getTemporaryTabWhitelistForTab(requestDetails.tabId);
            if (
